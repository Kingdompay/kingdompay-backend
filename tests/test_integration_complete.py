...
class CompleteIntegrationTest:
    ...
    def setup_app(self):
        ...
        os.environ["JWT_SECRET_KEY"] = os.environ.get(
            "JWT_SECRET_KEY", "test-jwt-secret-key-for-integration-test"
        )
    ...
    def print_step(self, step, message, status=None):
        ...
        icon = (
            "[PASS]" if status == "pass" else "[FAIL]" if status == "fail" else "[...]"
        )
        ...
        with self.app.app_context():
            ...
            try:
                ...
                if otp_result.get("success"):
                    ...
                    if not otp_code:
                        otp_record = (
                            OTPVerification.query.filter_by(phone_number=test_phone)
                    ...
                    if otp_code:
                        self.print_step(
                            "Step 2", f"OTP code retrieved: {otp_code}", None
                        )
                        ...
                        if verify_result.get("success"):
                            ...
                            self.print_step(
                                "Step 2", "OTP verified successfully", "pass"
                            )
                            ...
                            if user:
                                ...
                                self.print_step(
                                    "Step 3", f"User created: {user.id}", "pass"
                                )
                                ...
                                if wallet:
                                    ...
                                    self.print_step(
                                        "Step 4", f"Wallet created: {wallet.id}", "pass"
                                    )
                                else:
                                    self.print_step(
                                        "Step 4", "Wallet not found", "fail"
                                    )
                            else:
                    ...
                    self.print_step(
                        "Step 1",
                        f"OTP request failed: {otp_result.get('message')}",
                        "fail",
                    )
                ...
                if otp_result.get("success"):
                    ...
                    if otp_record:
                        ...
                        if otp_code_for_test:
                            self.print_step(
                                "Test 2",
                                f"OTP stored: {otp_code_for_test[:3]}***",
                                "pass",
                            )
                        else:
                        ...
                        if otp_code_for_test:
                            if OTPVerification.verify_otp(
                                test_phone, otp_code_for_test
                            ):
                                results["otp_validation"] = True
                                self.print_step(
                                    "Test 3", "OTP validation successful", "pass"
                                )
                            else:
                                self.print_step(
                                    "Test 3", "OTP validation failed", "fail"
                                )
                        else:
                            self.print_step(
                                "Test 3",
                                "OTP code not available for validation",
                                "pass",
                            )
                    ...
                    self.print_step(
                        "Test 1",
                        f"OTP generation failed: {otp_result.get('message')}",
                        "fail",
                    )
            except Exception as e:
    ...
    def test_mpesa_integration(self):
        ...
        with self.app.app_context():
            ...
            self.print_step(
                "Test 2", "Checking M-Pesa configuration from environment", None
            )
            try:
                ...
                if has_shortcode and has_passkey and has_callback:
                    ...
                    self.print_step(
                        "Test 2", "Configuration complete from environment", "pass"
                    )
                    print(f"   [OK] Shortcode: {self.stk_service.shortcode}")
                    print(f"   [OK] Callback URL: {self.stk_service.callback_url}")
                    print(f"   [OK] Base URL: {self.stk_service.base_url}")
                else:
                    self.print_step("Test 2", "Configuration incomplete", "fail")
                    print(
                        "   [WARN] M-Pesa credentials not fully configured in environment"
                    )
            ...
            try:
                ...
                if hasattr(self.stk_service, "initiate_stk_push"):
                    ...
                    print("   [WARN] Actual STK Push requires M-Pesa credentials")
                else:
        ...
        with self.app.test_client() as client:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.print_step(
                        "Test 1", f"Status: {response.status_code}", "pass"
                    )  # Fallback works
            except Exception as e:
                self.print_step("Test 1", f"Error: {str(e)}", "fail")
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.print_step(
                        "Test 2", f"Status: {response.status_code}", "pass"
                    )  # Fallback works
            except Exception as e:
                self.print_step("Test 2", f"Error: {str(e)}", "fail")
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.print_step(
                        "Test 3", f"Status: {response.status_code}", "pass"
                    )  # Fallback works
            except Exception as e:
                self.print_step("Test 3", f"Error: {str(e)}", "fail")
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.print_step(
                        "Test 4", f"Status: {response.status_code}", "pass"
                    )  # Fallback works
            except Exception as e:
                self.print_step("Test 4", f"Error: {str(e)}", "fail")
                ...
                if otp_result.get("success"):
                    ...
                    if verify_result.get("success"):
                        ...
                        if user:
                            ...
                            self.print_step(
                                "Flow 1", f"User registered: {user.id}", "pass"
                            )
                            ...
                            if wallet:
                                ...
                                self.print_step(
                                    "Flow 2", f"Wallet funded: {wallet.balance}", "pass"
                                )
                                ...
                                if payment.id:
                                    ...
                                    self.print_step(
                                        "Flow 4", "Recording transaction", None
                                    )
                                    ...
                                    if transaction.id:
                                        ...
                                        self.print_step(
                                            "Flow 4",
                                            f"Transaction recorded: {transaction.id}",
                                            "pass",
                                        )
            except Exception as e:
    ...
    def generate_summary(self, total_time):
        ...
        print("\nTest Results:")
        print(
            f"   Auth Flow: {'PASS' if self.results['auth_tests'].get('token_generation') else 'FAIL'}"
        )
        print(
            f"   OTP Tests: {'PASS' if all(self.results['otp_tests'].values()) else 'FAIL'}"
        )
        print(
            f"   M-Pesa Tests: {'PASS' if all(self.results['mpesa_tests'].values()) else 'FAIL'}"
        )
        print(
            f"   Template Tests: {'PASS' if all(self.results['template_tests'].values()) else 'FAIL'}"
        )
        print(
            f"   Full Flow: {'PASS' if all(self.results['full_flow_tests'].values()) else 'FAIL'}"
        )
