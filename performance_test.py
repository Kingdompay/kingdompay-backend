...
class PerformanceTester:
    ...
    def test_concurrent_users(self, num_users=50, operations_per_user=10):
        ...
        print(f"🔄 Testing {num_users} concurrent users with {operations_per_user} operations each...")
        
        start_time = time.time()
        results = []
        
        def user_operations(user_id):
            user_results = {'user_id': user_id, 'success': 0, 'errors': 0, 'operations': []}
                    ...
                    for i in range(operations_per_user):
                        ...
                        try:
                            ...
                            if operation == 'deposit':
                                ...
                                transactions = Transaction.query.filter_by(destination_wallet_id=wallet.id).count()
        ...
        self.results['load_test_results']['concurrent_users'] = {
            ...
            'avg_operation_time': statistics.mean(all_operations) if all_operations else 0,
        ...
        print(f"✅ Concurrent users test completed: {total_success}/{num_users * operations_per_user} successful")
        return total_success, total_errors
    
    def test_api_load(self, num_requests=200, concurrent_requests=10):
        """Test API load handling"""
        print(f"🔄 Testing API load: {num_requests} requests with {concurrent_requests} concurrent...")
        
        start_time = time.time()
        results = []
        
        def api_request(request_id):
            request_results = {'request_id': request_id, 'success': False, 'response_time': 0, 'status_code': 0}
        ...
        print(f"✅ API load test completed: {successful_requests}/{num_requests} successful")
    ...
    def test_database_scalability(self, num_records=1000):
        ...
        with self.app.app_context():
            try:
                ...
                complex_result = db.session.query(User, Wallet).join(Wallet).limit(100).all()
                complex_query_time = time.time() - complex_query_start
                
                operations_count += 1
                
                print(f"✅ Database scalability test completed: {operations_count} operations")
                
            except Exception as e:
                error_count += 1
                print(f"❌ Database scalability test failed: {str(e)}")
                db.session.rollback()
        ...
        self.results['scalability_tests']['database_scalability'] = {
            ...
            'complex_query_time': complex_query_time if 'complex_query_time' in locals() else 0
        }
    ...
    def test_memory_usage(self):
        ...
        self.results['memory_analysis'] = {
            ...
            'avg_memory_per_batch': statistics.mean([s['memory_increase'] for s in memory_snapshots]) if memory_snapshots else 0
        }
    ...
    def analyze_bottlenecks(self):
        ...
        print(f"✅ Bottleneck analysis completed: {len(bottlenecks)} bottlenecks identified")
    ...
    def generate_recommendations(self):
        ...
        if self.results['bottleneck_analysis']['high_impact'] > 0:
            recommendations.append("Address high-impact performance bottlenecks immediately")
        
        if self.results['bottleneck_analysis']['medium_impact'] > 0:
            recommendations.append("Plan optimization for medium-impact performance issues")
        ...
        if 'concurrent_users' in self.results['load_test_results']:
            ...
            if concurrent_data['success_rate'] < 95:
                ...
                recommendations.append("Review and optimize transaction isolation levels")
        ...
        if not recommendations:
            recommendations.append("Performance looks good! Consider implementing monitoring for production")
